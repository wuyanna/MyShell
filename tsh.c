...
/*
 * printBGJob - print the job that is sent to background
 */
void
printBGJob(struct job_t *job)
{
...
/*
 * cont_job - called by builtin commands fg, bg 
 * to change the state of a job
 */
int
cont_job(int bg, char *arg)
{
    int is_jobid;
    int jpid;
    char *jpid_str;
    ...
    job = is_jobid ? getjobjid(job_list, jpid) : getjobpid(job_list, jpid);
    job->state = bg ? BG : FG;
...
/*
 * builtin_command - handle builtin commands
 * Returns:
 *   1:     if the command is builtin command
 *   0:     other command
 */
int
builtin_command(struct cmdline_tokens tok)
{
    ...
    if (tok.builtins == BUILTIN_QUIT) {
        ...
        cont_job(0, tok.argv[1]);
        return 1;
    } else if (tok.builtins == BUILTIN_BG) {
        cont_job(1, tok.argv[1]);
        return 1;
    }
    return 0;
}
...
eval(char *cmdline) 
{
    ...
    if (!builtin_command(tok)) {
        pid_t pid = 0;
        // block signal until addjob to job list
        sigemptyset(&mask);
...
sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        ...
        if (WIFSTOPPED(status)) {
            ...
            if (verbose) {
                printf("sigchld_handler:Handler reaped child %d\n", (int)pid);
                fflush(stdout);
            }
